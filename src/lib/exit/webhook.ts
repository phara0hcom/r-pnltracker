 ...
 *
 * The same 2s line `SLOW_SERVER_FN_MS` draws for server functions, and drawn
 * again here because that one cannot reach this route: the alarm in
 * `src/start.ts` lives in a `type: 'function'` middleware, which wraps server
 * functions only. A route handler never runs it, whatever the header of that
 * file hoped.
 *
 * It matters more here than there. Every alert of the day fires at the same
 * close, each delivery holds a pooled connection for its whole round trip, and
 * TradingView gives up on an alert that answers too late — so a delivery that
 * has slowed is the difference between a bar arriving and a bar being lost,
 * with nothing on screen to say which happened.
 */
export const SLOW_FEED_DELIVERY_MS = 2000

/**
 * How a finished delivery ends up in Sentry.
 *
 * Declared here rather than imported from `db/exit.service.ts`, which owns the
 * enum this mirrors: `src/lib/` is DB-free, and the dependency belongs in that
 * direction. Drift is caught at the call site instead — the route hands this
 * function a `FeedDeliveryOutcome`, so a new member of the database enum stops
 * compiling there until it is handled here too.
 */
export type FeedOutcome = 'STORED' | 'UNKNOWN_TICKER' | 'INVALID_PAYLOAD'

/**
 * The channel a delivery deserves, decided in one place.
 *
 * Split out as a pure function for the reason `scrub.ts` and `offline/policy.ts`
 * are: the route is event glue, and "which of these is worth waking someone for"
 * is a policy that should be readable and tested on its own rather than inferred
 * from four scattered call sites.
 *
  ...
  switch (outcome) {
    ...
    case 'INVALID_PAYLOAD':
      return {
        ...
        fingerprint: ['tv-webhook', 'INVALID_PAYLOAD'],
      }

    /*
     * An alert exists for an instrument this account has never traded. Not
     * retryable and not a fault of the route, but it is always a real mistake in
     * the alert list, and it is silent: the bar is discarded and the Exit Rules
     * card just stays stale.
     */
    case 'UNKNOWN_TICKER':
      return {
        channel: 'warning',
        message: 'exit feed: no instrument for ticker',
        fingerprint: ['tv-webhook', 'UNKNOWN_TICKER'],
      }
    ...
    case 'STORED':
      return durationMs > SLOW_FEED_DELIVERY_MS
        ? {
            ...
            fingerprint: ['tv-webhook', 'slow-delivery'],
          }
        : /*
           * The ordinary case, and the one that must stay cheap. A bar landed in
           * time; that is context for whatever fails next, not an alert. The
